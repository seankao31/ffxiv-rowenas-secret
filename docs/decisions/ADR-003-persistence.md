...
Raw scan data stored as a typed TypeScript object in the process heap. Lost on restart; repopulated within ~1.5 min.
...
- The ~1.5 min cold-start repopulation time on restart is acceptable for a personal tool (per ADR-005 empirical validation).
