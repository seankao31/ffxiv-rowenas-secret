...
- The backend produces a complete updated ranked list only once per full scan cycle (~5 min with the per-world strategy — see ADR-005 and ADR-006). There are no meaningful partial updates between cycles to stream.
